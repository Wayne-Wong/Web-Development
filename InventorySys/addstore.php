...
{
	$query = "SELECT COUNT(addedbyuser) AS store_num FROM stores WHERE addedbyuser='".$_SESSION['username']."'";
	$result = mysql_query($query);
	$row = mysql_fetch_assoc($result);
	if($row['store_num'] >= 5){
		$status = "5 stores are maximum for your account.";
	} else {	
		...
		$ins_query="insert into stores(`store_name`,`address`,`city`,`state`,`zip`,`phone`,`addedbyuser`)values('$store_name','$address','$city','$state','$zip','$phone','$addedbyuser')";
		mysql_query($ins_query) or die(mysql_error());
		$status = "Store Inserted Successfully.";
	}
}
?>
<!DOCTYPE html>
...
<br>
 ...
 <div class="container">
                	<div class="storeinfo">
					<p style="font-size:200%;">Store Information</p>
					</div>
</div>
<center>
<div class="form">
...
<input type="hidden" name="new" value="1" />
    <table width="90%" border="0" cellspacing="0" cellpadding="6">
      <tr>
        ...
        <td width="80%"><label>
          <input name="store_name" type="text" size="64" />
        </label></td>
      </tr>
		<tr>
      ...
      <tr>
        <td>&nbsp;</td>
        <td><label>
          <input type="submit" class="btn btn-default btn-md" name="button" value="Add This Store Now" />
        </label></td>
      </tr>
    </table>
    </form>
